defmodule CustyardWeb.WebhookControllerTest do
  ...
  describe "inbound/2 (legacy webhook)" do
    setup %{conn: conn} do
      # Set the webhook token for auth
      Application.put_env(:custyard, :webhook_token, "test-webhook-token")

      on_exit(fn ->
        Application.delete_env(:custyard, :webhook_token)
      end)
    ...
    test "returns 401 without authorization header", %{conn: conn} do
      response =
        conn
        |> post(~p"/api/webhook/inbound", %{})
        |> json_response(401)

      assert response["error"] =~ "Missing authorization"
    end
    ...
    test "returns 401 with invalid token", %{conn: conn} do
      ...
      assert response["error"] =~ "Invalid authorization"
    end

    test "processes valid email payload", %{conn: conn} do
      ...
      params = %{
        ...
        "text" => "Test body",
        "headers" => %{
          "message-id" => "<unique-#{System.unique_integer([:positive])}@test.com>"
        }
      }

      response =
        conn
        |> put_req_header("authorization", "Bearer test-webhook-token")
        |> post(~p"/api/webhook/inbound", params)
        |> json_response(200)
  ...
  describe "inbound/2 legacy deprecation" do
    setup %{conn: conn} do
      Application.put_env(:custyard, :webhook_token, "test-webhook-token")
    ...
    test "returns x-deprecated response header", %{conn: conn} do
      org = insert_organization()
      _contact = insert_contact(organization_id: org.id, email: "alice@test.example.com")
      ...
      params = %{
        "from" => "alice@test.example.com",
        "to" => "support@example.com",
        "subject" => "Deprecation header test",
        "text" => "Checking for x-deprecated header",
        "headers" => %{
          "message-id" => "<deprecated-#{System.unique_integer([:positive])}@test.com>"
        }
      }
      ...
      assert get_resp_header(conn, "x-deprecated") == ["true"]
    end

    test "legacy endpoint still processes emails correctly", %{conn: conn} do
      org = insert_organization()
      _contact = insert_contact(organization_id: org.id, email: "alice@test.example.com")
      ...
      params = %{
        "from" => "alice@test.example.com",
        "to" => "support@example.com",
        "subject" => "Legacy still works",
        "text" => "This should create a conversation even though deprecated",
        "headers" => %{
          "message-id" => "<legacy-ok-#{System.unique_integer([:positive])}@test.com>"
        }
      }
      ...
      # Verify the conversation was actually created with the correct data
      conversation = Repo.get!(Custyard.Conversation, response["conversation_id"])
      assert conversation.subject == "Legacy still works"
